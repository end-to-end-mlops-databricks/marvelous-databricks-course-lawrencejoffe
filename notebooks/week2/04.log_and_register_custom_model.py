...
mlflow.search_runs(
    experiment_names=["/Shared/lj_loan_prediction"],
...
wrapped_model = LoanPredictionModelWrapper(model) # we pass the loaded model to the wrapper
...
mlflow.set_experiment(experiment_name="/Shared/lj_loan_prediction-pyfunc")
...
loaded_model = mlflow.pyfunc.load_model(f'runs:/{run_id}/pyfunc-model')
loaded_model.unwrap_python_model()

# COMMAND ----------

model_name = f"{catalog_name}.{schema_name}.loan_prediction_model_pyfunc"

model_version = mlflow.register_model(
    model_uri=f'runs:/{run_id}/pyfunc-model',
    name=model_name,
