"""
This script evaluates and compares a new loan prediction model against the currently deployed model.
Key functionality:
...
# config_path = ("/Volumes/mlops_test/loan_predictions/data/project_config.yml")
...
serving_endpoint_name = "lj-loan-predictions-model-serving-fe"
...
# test_set = test_set.withColumn("YearBuilt", col("YearBuilt").cast("int"))

# Select the necessary columns for prediction and target
X_test_spark = test_set.select(num_features + cat_features + ["Id"])
...
df = df.withColumn("error_old", F.abs(df["SalePrice"] - df["prediction_old"]))
...
evaluator = RegressionEvaluator(labelCol="SalePrice", predictionCol="prediction_new", metricName="rmse")
...
if mae_new < mae_old:
    ...
    model_version = mlflow.register_model(
        ...
        name=f"{catalog_name}.{schema_name}.loan_predictions_model_fe",
